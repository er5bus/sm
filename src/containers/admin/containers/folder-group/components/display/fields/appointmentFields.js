...
import {eventContextUIHelper, eventGroupNatureUIHelper, eventStatusUIHelper, eventThemeUIHelper, FOLDER_EXIT_COURSE} from "../../../../../UIHelpers";
...
export const appointmentFields = ({ intl }) => [
  ...
  {
    name: "folders",
    openShowPage: ({ id, status }, history) => status !== FOLDER_EXIT_COURSE && history.push(routes.folderShow.path.replace(":param", id)),
    addAnotherActions: [
      {
        label: intl.formatMessage({ id: "GENERAL.ADD_EVENT" }),
        iconPath: "/media/svg/icons/Navigation/Plus.svg",
        onShowCondition: ({ hasEvent, status }) => !hasEvent && status !== FOLDER_EXIT_COURSE,
