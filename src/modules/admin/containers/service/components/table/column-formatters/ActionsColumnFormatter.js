...
const ActionsColumnFormatter = (cellContent, row, rowIndex, formatExtraData) => {
  ...
  return (
      ...
      {row.isActive && (
        ...
        >
          <ProtectedLink rule={formatExtraData.editServiceRule}>
            <a
              className="btn btn-sm btn-icon btn-light btn-hover-warning btn-sm mx-1 my-1"
              onClick={() => formatExtraData.openEditServicePage(row.id)}
            >
        ...
        <OverlayTrigger
          overlay={
            <Tooltip id="partnership-show-tooltip">
              <FormattedMessage id="SERVICE.SHOW" />
            </Tooltip>
          }
        >
          <ProtectedLink rule={formatExtraData.showServiceRule}>
            <a
              className="btn btn-sm btn-icon btn-light btn-hover-primary btn-sm mx-1 my-1"
              onClick={() => formatExtraData.openShowServicePage(row.id)}
            >
        ...
        <OverlayTrigger
          overlay={
            <Tooltip id="partnership-disable-tooltip">
              <FormattedMessage id="SERVICE.DISABLE" />
            </Tooltip>
          }
        >
          <ProtectedLink rule={formatExtraData.deactivateServiceRule}>
            <a
              className="btn btn-sm btn-icon btn-light btn-hover-danger btn-sm my-1 mx-1"
              onClick={() => formatExtraData.openDeactivateServiceDialog(row.id)}
            >
        ...
        <OverlayTrigger
          overlay={
            <Tooltip id="partnership-enable-tooltip">
              <FormattedMessage id="SERVICE.ENABLE" />
            </Tooltip>
          }
        >
          <ProtectedLink rule={formatExtraData.activateServiceRule}>
            <a
              className="btn btn-sm btn-icon btn-light btn-hover-success btn-sm mx-1 my-1"
              onClick={() => formatExtraData.openActivateServiceDialog(row.id)}
            >
