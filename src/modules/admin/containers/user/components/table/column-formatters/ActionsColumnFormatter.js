...
const ActionsColumnFormatter = (
  ...
  <div className="text-right">
    ...
    {!row.isActive && !row.isDeleted && (
      ...
      >
        <ProtectedLink rule={formatExtraData.deleteUserRule}>
          <a
            className="btn btn-sm btn-icon btn-light btn-hover-danger btn-sm mx-1 my-1"
            onClick={() => formatExtraData.openDeleteUserDialog(row.id)}
          >
            <span className="svg-icon svg-icon-md svg-icon-danger">
    ...
    {row.isDeleted && (
      <OverlayTrigger
        overlay={
          <Tooltip id="user-enable-tooltip">
            <FormattedMessage id="USER.UNDELETE" />
          </Tooltip>
        }
      >
        <ProtectedLink rule={formatExtraData.undeleteUserRule}>
          <a
            className="btn btn-sm btn-icon btn-light btn-hover-success btn-sm mx-1 my-1"
            onClick={() => formatExtraData.openUndeleteUserDialog(row.id)}
          >
            <span className="svg-icon svg-icon-md svg-icon-success">
